...
from services.email_service import enviar_mail_aInspeccionar
from services.email_service import enviar_mail_aUrbanismo
from services.email_service import enviar_mail_carpetaIncompleta
from services.email_service import enviar_mail_derechoConstruccionAPagar
from services.email_service import enviar_mail_expedienteACatastro
from services.email_service import enviar_mail_expedienteConDeuda
from services.email_service import enviar_mail_expedienteFinalizado
...
def enviarMailSegunEstado(idEstadoExpediente: int, expediente: ExpedienteModel, background_tasks):
    try:
        print(f" Estado recibido: {idEstadoExpediente}")
        
        destinatario = "nidia@9dejulio.gov.ar"  # después dinámico

        # Validar si el formato despues del @ es valido
        if not es_email_valido(destinatario):
            print(f"Email inválido, no se envía: {destinatario}")
            return

        idEstadoExpediente = int(idEstadoExpediente)
        
        # Mapa de estados → funciones
        mapa_mails = {
        ...
        if not funcion_mail:
            print("Estado no válido, no se envía mail")
            return

        # Enviar en background
        background_tasks.add_task(
            funcion_mail,
            "nidia@9dejulio.gov.ar",  # después esto lo podés hacer dinámico
            expediente
        )

        print("Mail agregado a BackgroundTasks")

    except Exception as e:
        print(f"Error al preparar el envío del mail: {e}")
...
async def agregar_expediente_post(
    ...
    session: Session = Depends(get_session)
        ...
        if parte1 == "duplicado":
            ...
            try:
                enviarMailSegunEstado(idEstadoExpediente,nuevo_expediente,background_tasks)
                '''background_tasks.add_task(
            ...
            return JSONResponse(
                ...
                content={"message": "El Expediente  se ha agregado exitosamente y se ha enviado el mail."}
            )
    ...
    if exito == "noExiste":
            ...
            try:
                if idEstadoExpedienteAnterior != idEstadoExpedienteNuevo:
                    ...
                    enviarMailSegunEstado(idEstadoExpedienteNuevo,expediente_actualizado,background_tasks)
                    
                '''background_tasks.add_task(
                    enviar_expediente_creado,
                    "nidia@9dejulio.gov.ar",
                    expediente.nroLegajo,
                    expediente.anioMesaEntrada,
                    expediente.nroPartida
                )
            '''
            except Exception as e:
