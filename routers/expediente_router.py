...
def parse_profesional_string(valor: str):
    ...
    return {
        ...
        "email": partes[2] if len(partes) > 2 else None
    }

# ------------------------------------
# VALIDAR PARTIDA (AJAX) CONTRA RAFAM
# ------------------------------------
@router.get("/validar_partida")
async def validar_partida(nroPartida: str):

    resultado = OracleService.obtener_partida(nroPartida)

    if not resultado["existe"]:
        print("NO existe PARTIDA")
        return {
            "existe": False,
            "mensaje": f"La partida {nroPartida} no existe en RAFAM."
        }
...
async def agregar_expediente_post(
    ...
    session: Session = Depends(get_session)
        ...
        # PROPIETARIO PRINCIPAL: Me quedo con el marcado como principal paar enviar el mail correspondiente.
        propietario_principal = None

        for p_dict in valoresPropietarios:
            if p_dict.get("figuraPpal", "") == 1:
                propietario_principal = p_dict["propietario"]
                break

        print("=====================================================")
        print("propietario PPla");
        print(propietario_principal);
        print(propietario_principal.email)
        print("=====================================================")

        if not propietario_principal:
            return JSONResponse(
                status_code=status.HTTP_400_BAD_REQUEST,
                content={"message": "Debe marcar al menos un Propietario como Contacto Ppal?"}
            )
        
        # VALIDACIÓN: CUIL duplicado en el FORM con distinto apellido
        cuits_map = {}

        for p_dict in valoresPropietarios:
            p = p_dict["propietario"]
            cuil = p.cuil_cuit.strip()
            apellido = p.apellido.strip().lower()

            if cuil in cuits_map:
                if cuits_map[cuil] != apellido:
                    return JSONResponse(
                        status_code=status.HTTP_400_BAD_REQUEST,
                        content={"error": f"El Cuil/Cuit {cuil} fue ingresado con distintos apellidos en el formulario."
                        }
                    )
            else:
                cuits_map[cuil] = apellido

        # -------------------------
        # PROFESIONALES
        # -------------------------
        valoresExpedientesProfesionales = []
        emailProfesional = None
        profesional_principal = None
        ...
        if idFilaProf > 1 :
            for i in range(1, idFilaProf + 1):
                ...
                # PROFESIONAL PRINCIPAL: Me quedo con el marcado como principal para enviar el mail correspondiente.
                #Si hay mas de uno toma uno solo
                if parsed and parsed.get("contactoPpal") == 1:
                    emailProfesional = parsed.get("email")
                    profesional_principal = p
                    break
        ...
        if not profesional_principal:
            return JSONResponse(
                status_code=status.HTTP_400_BAD_REQUEST,
                content={"message": "Debe marcar al menos un Profesional como Contacto PPal?"}
            ) 
              
        
        print("=====================================================")
        print("Validar Partida");
        print("=====================================================")
        
        # VALIDAR PARTIDA EN ORACLE - Se vuelve a validar antes de agregar el expediente
        #existe_partida = OracleService.existe_partida(nroPartida)
        resultado = OracleService.obtener_partida(nroPartida)
        existe_partida = resultado["existe"]
        ...
        nuevo_expediente = ExpedienteModel(
            ...
            catastroRafam = catastroRafamHidden,
            sucesion=sucesion,
        ...
        if parte1 == "duplicado":
            return JSONResponse(
                ...
                content={"error": "El Cuil/Cuit " + parte2 + " ingresado ya se encuentra asignado a un Propietario registrado pero el Apellido no coincide con el resgistrado. Verifique la información."}
            )
        else:
            # Enviar correo una vez agregado el expediente y sus relaciones
            #Tomo el propietario ppal para enviar los datos
            emailPropietario = propietario_principal.email
            celularPropietario = f"{propietario_principal.areaCelular}-{propietario_principal.nroCelular}" 

            resultado_mail = enviarMailSegunEstado(idEstadoExpediente, nuevo_expediente, background_tasks, emailPropietario, celularPropietario, emailProfesional)
